 ...
 * Lead Author(s):
 * @author Chris Jain
 * @author Period 1 Student Name
 * 
 * Responsibilities of class:
 * 
 * Helper methods for the Candy Crush game to manipulate
 * the grid of Candy objects
 */
...
public class Grid {
	...
	private void fillGrid() { 
		...
		for( int row = 0; row < this.grid.length ; row++ ) {
			...
			for( int col = 0; col < this.grid[row].length; col++) {
				///random color gen 
				String randomColor = this.randomColor();
				grid[row][col] = new Candy(randomColor);
			}
	...
	public int[] findMaximum(Candy[] candies) { 
	    ...
	    for( int i = 0; i < candies.length; i ++ ) {
	    		...
	    		else if( candies[i].getColor().equals(candies[i+1].getColor()) ) {
	    			count++;
	    			if ( count > longest) {
	    				longest = count;
	    				index = i - count + 1 ;
	...
	public boolean isAdjacent(ArrayPosition p1, ArrayPosition p2) {
		...
		if( ((p1.r == p2.r+1) || (p1.r ==  p2.r - 1)) && p1.c == p2.c ) {
			x = true;
		}
